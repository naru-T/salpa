...
# Get reference DEM
dem <- elevatr::get_elev_raster(bbox_sf, z = 12, clip = "bbox")
dem_3857 <- raster::projectRaster(dem, crs = 3857)
dem_rast <- terra::rast(dem_3857)

# Extract DEM values for comparison
original_dem <- terra::extract(dem_rast, original_sf)
names(original_dem) <- c("ID", "dem")
linearly_adjusted_dem <- terra::extract(dem_rast, linearly_adjusted_sf)
names(linearly_adjusted_dem) <- c("ID", "dem")
```

## Step 3: Positional Adjustment

After linear alignment, we can further refine the point positions using genetic algorithm optimization. By default, the Euclidean distance metric is used, but you can specify other metrics based on your needs:
...
# Alternative: Using DTW distance
corrected_positions_dtw <- positional_correction(
  lidar_footprints = lidar_footprints_linearly_adjusted,
  input_rast = dem_rast,
...
You can compare the results of different distance metrics:

```{r compare_metrics, eval=FALSE}
# Function to calculate metrics
calculate_metrics <- function(positions, dem_rast, lidar_value) {
    dem_values <- terra::extract(dem_rast, positions)
    lidar_values <- positions[[lidar_value]]
    
    # Calculate various metrics
    metrics <- perform_distance(lidar_values, dem_values$mean)
    return(metrics)
}

# Compare results
euclidean_metrics <- calculate_metrics(corrected_positions[[2]], dem_rast, "elev_lowestmode")
dtw_metrics <- calculate_metrics(corrected_positions_dtw[[2]], dem_rast, "elev_lowestmode")

print(paste("Euclidean Distance:", euclidean_metrics$distance))
print(paste("DTW Distance:", dtw_metrics$distance))
```
...
difference_df <- data.frame(
    ...
    original = as.numeric(lidar_footprints_linearly_adjusted$elev_lowestmode) - 
              as.numeric(original_dem$dem),
    linearly_adjusted = as.numeric(lidar_footprints_linearly_adjusted$elev_lowestmode) - 
                      as.numeric(linearly_adjusted_dem$dem),
    positionally_corrected = as.numeric(lidar_footprints_linearly_adjusted$elev_lowestmode) - 
                      as.numeric(corrected_positions_dem$dem)
)
...
scatter_df <- data.frame(
    reference = as.numeric(lidar_footprints_linearly_adjusted$elev_lowestmode),
    raw = as.numeric(original_dem$dem),
    linear = as.numeric(linearly_adjusted_dem$dem),
    location = as.numeric(corrected_positions_dem$dem)
) %>%
