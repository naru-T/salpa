...
The first step is to align the LiDAR points to their best-fit line using an analytical total least squares approach. In this method, for each shot group (identified by the first 12 characters of the 'shot_number'), the function computes the principal component (direction of maximum variance) and projects the points onto the line passing through the group centroid. This adjustment corrects systematic linear offsets in the data and stores the original coordinates in 'orig_x' and 'orig_y'.

```{r linear_alignment, eval=TRUE}
...
```{r positional_adjustment, eval=TRUE}
...
# Run positional correction optimization
...
corrected_positions_dem <- terra::extract(dem_rast, corrected_positions[[2]])
names(corrected_positions_dem) <- c("ID", "dem")
```
