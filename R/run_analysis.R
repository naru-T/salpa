...
#' @export
run_analysis <- function(lidar_footprints, input_raster, minimizing_method = "dtw", target_variable, buf = 12.5, crs_code, lidar_value, lower_bounds = c(-30, -30), upper_bounds = c(30, 30), pop_size = 50, max_iter = 100, parallel = parallel) {
    optim_result <- tryCatch({
        results <- minimize_loss(lidar_footprints, input_raster, minimizing_method, target_variable, buf, crs_code, lidar_value, lower_bounds, upper_bounds, pop_size, max_iter, parallel)
