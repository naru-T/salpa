...
#' @param input_rast raster object for extraction
...
#' @export
extract_values <- function(buffered_data, input_rast, target_variable){
    extracted_values <- exact_extract(input_rast, buffered_data, target_variable, progress = FALSE)
    out <- buffered_data |>
            mutate(
